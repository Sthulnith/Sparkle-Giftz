...
const SUPABASE_URL     = (import.meta.env.VITE_SUPABASE_URL as string) || 'https://lccruzpszbcwzmfdjyyj.supabase.co';
const SUPABASE_ANON_KEY = (import.meta.env.VITE_SUPABASE_ANON_KEY as string) || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxjY3J1enBzemJjd3ptZmRqeXlqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODQ2MTY3NjQsImV4cCI6MjEwMDE5Mjc2NH0.WdpqI0Dl7CZmt9ziWLlkbBjcoaYoo2X0Qr9BIgqS2do';
...
export async function createOrder(input: {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      order_number,
      customer_name: input.customer_name ? (input.customer_name.length > 90 ? input.customer_name.slice(0, 87) + '...' : input.customer_name) : '',
      phone: input.phone ? input.phone.slice(0, 30) : '',
      email: input.email ? input.email.toLowerCase().trim().slice(0, 90) : '',
      address: input.address,
      city: input.city ? (input.city.length > 90 ? input.city.slice(0, 87) + '...' : input.city) : '',
      ...
      wrapping: input.wrapping ? (input.wrapping.length > 90 ? input.wrapping.slice(0, 87) + '...' : input.wrapping) : null,
